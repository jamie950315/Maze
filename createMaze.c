...
char corner[16][4] = {" ","╶","╴","─","╷","┌","┐","┬","╵","└","┘","┴","│","├","┤","┼"}; // its UTF-8 each character is of 3 bytes plus 1 byte for null character
////////////////////////////////////////////////////////////////////////////////////
...
for (int y = 0; y < maze_h; y++) {
    for (int x = 0; x < maze_w; x++) {
        struct cell* m = &maze1->c[(y * maze_w) + x];
        if (m->left_wall || m->right_wall || m->top_wall || m->bottom_wall) {
            printf("Wall at: (%d, %d)\n", y, x);
        }
    }
}*/
